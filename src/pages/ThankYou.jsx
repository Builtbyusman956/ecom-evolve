    ...
    <section style={{ background:'var(--canvas)', minHeight:'100vh', paddingTop:120, paddingBottom:96 }}>
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'0 1.5rem' }}>
        <motion.div variants={up(0.05)} initial="hidden" animate="show" style={{ maxWidth:560, marginBottom:56 }}>
          <span style={{ display:'inline-block', fontFamily:'var(--font-mono)', fontSize:11, letterSpacing:'0.20em', textTransform:'uppercase', padding:'6px 14px', borderRadius:100, background:'var(--violet-muted)', border:'1px solid var(--violet-border)', color:'var(--violet)', marginBottom:18 }}>
            What We Do
          </span>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))', gap:18 }}>
          {SVCS.map(({ icon:Icon, title, desc }, i) => (
            <motion.div key={title} variants={up(0.1+i*0.07)} initial="hidden" animate="show"
              style={{ background:'#fff', borderRadius:18, border:'1px solid rgba(0,0,0,0.07)', padding:'28px 28px 28px', boxShadow:'0 1px 4px rgba(0,0,0,0.05)', cursor:'default', transition:'all 0.25s' }}
              onMouseEnter={e=>{ e.currentTarget.style.boxShadow='0 4px 24px rgba(108,71,255,0.12)'; e.currentTarget.style.borderColor='rgba(108,71,255,0.20)'; e.currentTarget.style.transform='translateY(-3px)' }}
              onMouseLeave={e=>{ e.currentTarget.style.boxShadow='0 1px 4px rgba(0,0,0,0.05)'; e.currentTarget.style.borderColor='rgba(0,0,0,0.07)'; e.currentTarget.style.transform='translateY(0)' }}
            >
              <div style={{ width:44, height:44, borderRadius:12, background:'var(--violet-muted)', border:'1px solid var(--violet-border)', display:'flex', alignItems:'center', justifyContent:'center', marginBottom:18 }}>
                <Icon size={20} color="var(--violet)" />
              </div>
              <h3 style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:'1.2rem', color:'var(--ink)', marginBottom:10 }}>{title}</h3>
              <p style={{ fontFamily:'var(--font-body)', fontSize:14, color:'var(--ink-soft)', lineHeight:1.65 }}>{desc}</p>
            </motion.div>
          ))}
        </div>

        <motion.div variants={up(0.7)} initial="hidden" animate="show" style={{ marginTop:56, textAlign:'center' }}>
          <p style={{ fontFamily:'var(--font-body)', fontSize:14, color:'var(--ink-muted)', marginBottom:20 }}>Not sure which service you need? Let's talk.</p>
